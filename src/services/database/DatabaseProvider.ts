...
export interface DatabaseProvider {
  ...
  saveContractor(contractor: Partial<Contractor> & { id?: string; companyName: string; inn: string; address: string; }): Promise<Contractor>;
  deleteContractor(id: string): Promise<void>;

  // Drivers
  getDrivers(): Promise<Driver[]>;
  saveDriver(driver: Partial<Driver> & { id?: string; name: string; phone: string; }): Promise<Driver>;
  deleteDriver(id: string): Promise<void>;

  // Vehicles
  getVehicles(): Promise<Vehicle[]>;
  saveVehicle(vehicle: Partial<Vehicle> & { id?: string; brand: string; model: string; licensePlate: string; }): Promise<Vehicle>;
  deleteVehicle(id: string): Promise<void>;

  // Routes
  getRoutes(): Promise<Route[]>;
  saveRoute(route: Partial<Route> & { id?: string; name: string; pointA: string; pointB: string; }): Promise<Route>;
  deleteRoute(id: string): Promise<void>;

  // CargoTypes
  getCargoTypes(): Promise<CargoType[]>;
  saveCargoType(cargoType: Partial<CargoType> & { id?: string; name: string; }): Promise<CargoType>;
  ...
  saveTrip(trip: Partial<Trip> & { id?: string /* other required fields for save */ }): Promise<Trip>;
  deleteTrip(id: string): Promise<void>;

  // TripExpenses
  getTripExpenses(tripId: string): Promise<TripExpense[]>;
  createTripExpense(expense: Omit<TripExpense, 'id' | 'createdAt' | 'updatedAt' | 'userId'> & { tripId: string }): Promise<TripExpense>;
